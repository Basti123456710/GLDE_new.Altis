...
switch (_code) do
{
	...
	case _mapKey:
	{
		switch (playerSide) do 
		{
			...
			case civilian: {if(!visibleMap) then {[] spawn life_fnc_gangPlayerMarkers;}};
		};
	};
	...
	case 33:
	{
		...
		if(playerSide == west && vehicle player != player && !_alt && _ctrlKey && !life_siren2_active) then
		{
			...
			{
				titleText ["Emergency Sirens Off","PLAIN"];
				_veh setVariable["siren2",false,true];
			}
				else
			{
				titleText ["Emergency Sirens On","PLAIN"];
			...
			if(_veh isKindOf "House_F" && playerSide == civilian) then {
				...
				if(_veh in life_vehicles && player distance _veh < 8) then {
					if(_locked == 2) then {
						...
						[[_veh,"car_lock"],"life_fnc_say3D",nil,true] spawn life_fnc_MP;
					} else {
						if(local _veh) then {
							_veh lock 2;
						} else {
						...
						[[_veh,"car_unlock"],"life_fnc_say3D",nil,true] spawn life_fnc_MP;
					};
				};
			};
		};
